...
func sortCompaniesInReadme(readmePath string) error {
	...
	tableStart, tableEnd, err := findTableBoundaries(lines)
	if err != nil {
		return err
	}

	companies, err := extractCompanies(lines, tableStart, tableEnd)
	if err != nil {
		return err
	}

	sortCompanies(companies)
	newLines := rebuildFileContent(lines, companies, tableStart, tableEnd)

	if err := writeFile(readmePath, newLines); err != nil {
		return err
	}

	fmt.Printf("✅ %d companies sorted successfully!\n", len(companies))
	return nil
}
...
func sortCompanies(companies []Company) {
	sort.Slice(companies, func(i, j int) bool {
		nameI := normalizeName(extractCompanyName(companies[i].NameLink))
		nameJ := normalizeName(extractCompanyName(companies[j].NameLink))
		return nameI < nameJ
	})
}
...
func main() {
	readmePath := readmeFileName

	if _, err := os.Stat(readmePath); os.IsNotExist(err) {
		log.Fatalf("❌ %s", errFileNotFound)
	}

	fmt.Println("🔄 Sorting companies in README.md...")

	if err := sortCompaniesInReadme(readmePath); err != nil {
		log.Fatalf("❌ Sorting error: %v", err)
	}
